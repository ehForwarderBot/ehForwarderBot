...
class client:
    ...
    def check_login(self, uuid=None):
        ...
        if data and data.group(1) == '200':
            ...
            self.loginInfo['url'] = self.loginInfo[
                'url'][:self.loginInfo['url'].rfind('/')]
            self.loginInfo['BaseRequest'] = {}
            for node in xml.dom.minidom.parseString(r.text).documentElement.childNodes:
                if node.nodeName == 'skey':
                    self.loginInfo['skey'] = self.loginInfo['BaseRequest'][
                        'Skey'] = str(node.childNodes[0].data)
                elif node.nodeName == 'wxsid':
                    self.loginInfo['wxsid'] = self.loginInfo[
                        'BaseRequest']['Sid'] = str(node.childNodes[0].data)
                elif node.nodeName == 'wxuin':
                    self.loginInfo['wxuin'] = self.loginInfo[
                        'BaseRequest']['Uin'] = str(node.childNodes[0].data)
                elif node.nodeName == 'pass_ticket':
                    self.loginInfo['pass_ticket'] = self.loginInfo[
                        'BaseRequest']['DeviceID'] = str(node.childNodes[0].data)
            return '200'
    ...
    def __produce_msg(self, l):
        ...
        # 40 msg, 50 VOIPMSG, 52 voipnotifymsg, 53 webwxvoipnotifymsg, 9999
        # sysnotice
        for m in l:
            if '@' in m['FromUserName']:
                if not self.storageClass.find_nickname(m['FromUserName']):
                    self.get_contract(True)
                m['FromRemarkName'] = self.storageClass.find_remarkname(m['FromUserName'])
                m['FromNickName'] = self.storageClass.find_nickname(m['FromUserName'])
            if '@@' in m['FromUserName']:
            ...
            if m['MsgType'] == 1:  # words
                if m['Url']:
                    ...
                    msg = {
                        'Type': 'Text',
                        'Text': tools.escape_emoji(m['Content']), }
    ...
    def __produce_group_chat(self, msg):
        ...
        ChatRoomNickName = ''
        for i in self.storageClass.chatroomList:
            if i['UserName'] == msg['FromUserName']:
                ChatRoomNickName = tools.escape_emoji(i['NickName'])
        ...
        ActualNickName = tools.escape_emoji(self.storageClass.groupDict[
            msg['FromUserName']][ActualUserName]['NickName'])
        ActualDisplayName = tools.escape_emoji(self.storageClass.groupDict[
            msg['FromUserName']][ActualUserName]['DisplayName'])
        additionalItems = {
